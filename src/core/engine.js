...
export class Engine {
  ...
  _ensureComposer() {
    ...
    // A high threshold is what keeps this an accent rather than a haze: only the eyes, a crew
    // member's reflective hi-vis ring (steadily, for the brighter statuses; crossing it and
    // back is what makes an errored thread's ring a beacon rather than a wash) and the sun's
    // disc clear it, so lit surfaces stay crisp.
